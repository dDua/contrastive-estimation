...
class DROPDatasetBaselineReader(HotpotQADataBase):
    ...
    def extract_answer_info_from_annotation(self, answer_annotation: Dict[str, Any]) -> Tuple[str, List[str]]:
        answer_type = None
    ...
    def get_instance(self, example):
        """Return a single (or multiple) instance(s) for the given example.

        example: Dictionary containing {"passage": passage, "qa_pairs": qa_pairs, "mode": mode}

        Returns:
        -------
        all_instances: List of instances where each instance is a Dictionary. The dictionary needs to contain keys
            that the model needs as inputs; self.model_inputs, defined above. This function does not output the
            "attention_mask" with each instance, that is taken care of in the `build_segments` function.
        """
        passage = example["passage"]
        qa_pairs = example["qa_pairs"]
        mode = example["mode"]

        max_passage_len = (self.args.max_context_length - int(self.args.max_question_length) -
                           int(self.args.max_output_length))
        line = passage if not self.args.lowercase else passage.lower()
        full_context_ids = self.tokenizer.encode_plus(line)["input_ids"][:max_passage_len]
        context_info = {"tokens": full_context_ids, "sentence_offsets": [len(full_context_ids)]}
        ...
        for qa_pair in qa_pairs:
            ...
            answer = " <multi> ".join(answer_texts)
            ...
            question_tokens = self.tokenizer.encode_plus(question,
            ...
            input_ids = [[self.special_token_ids[0]] + context_info["tokens"] + question_tokens]

            qid = 00
            try:
                qid = int(qa_pair["query_id"])
            except Exception:
                pass

            all_instances.append({"input_ids": input_ids,
                                  "answer_input": answer_tokens[:-1],
                                  "answer_mask": [1]*(len(answer_tokens)-1),
