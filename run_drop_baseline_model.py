...
from configs.t5_drop_config import get_arguments as get_arguments_drop
...
def train():
    args, logger = get_arguments_drop()
    queue = Queue()
    ...
    tokenizer_class, model_class, dataset_class = T5Tokenizer, T5QA, DROPDatasetBaselineReader
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            if random.uniform(0, 1) <= args.output_prob:
                ...
                try:
                    gen_eos_idx = generated_ans_indices[0].index(eos_symbol)
                except Exception:
                    gen_eos_idx = -1
