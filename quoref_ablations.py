...
def train():
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, decoder_input_ids=output_src,
                        lm_labels=output_tgt, decoder_attention_mask=output_mask, max_len=20,contrast_labels=contrast_labels,
                        generate_answer=True)
            generate_indices, generated_probs = outputs[0], outputs[1]
            if len(outputs) == 5:
                contrast_logits = outputs[-1]
            else:
                contrast_logits = []

            answer_logits_masked = torch.masked_select(generated_probs[:,:num_org_q,:ans_len,:].reshape(-1, generated_probs.size(-1)),
                                               output_mask[:,:num_org_q].reshape(-1, 1).bool()).view(-1, generated_probs.size(-1))
            answer_labels_masked = torch.masked_select(output_tgt[:,:num_org_q].reshape(-1),
                                                       output_mask[:,:num_org_q].reshape(-1).bool())

            generate_indices = generate_indices.tolist()
            output_src[output_src == -100] = 0
            em, f1 = [], []
            for k in range(num_org_q):
                em_k, f1_k = [] , []
                for b in range(batch_size):
                    original_answer_k = tokenizer.decode(output_src[b][k].tolist(), skip_special_tokens=True,
                                                         clean_up_tokenization_spaces=True).lower().strip()
                    if eos_symbol in generate_indices[b][k]:
                        out_end_len = generate_indices[b][k].index(eos_symbol)
                    else:
                        out_end_len = -1
                    generated_answer_k = tokenizer.decode(generate_indices[b][k][:out_end_len], skip_special_tokens=True,
                                                          clean_up_tokenization_spaces=True).lower().strip()
                    em_k.append(get_exact_match(generated_answer_k, original_answer_k))
                    f1_k.append(get_f1(generated_answer_k, original_answer_k))
                em.append(em_k)
                f1.append(f1_k)
            ...
            predicted_ranks, gold_ranks = [], []
            for i, logit in enumerate(contrast_logits[:num_org_q]):
                predicted_ranks.append(logit.argmax().item())
                gold_ranks.append(i)
            predicted_ranks = torch.tensor(predicted_ranks)
            gold_ranks = torch.tensor(gold_ranks)
            predictions = (predicted_ranks == gold_ranks).long()
            final_ranks = torch.ones(num_org_q)

            if len(predictions) == 0:
                predictions = torch.zeros(1)
                final_ranks = torch.zeros(1)

            return (answer_logits_masked, predictions, torch.tensor(em).long().view(-1), 1+torch.tensor(f1).view(-1)),\
                   (answer_labels_masked, final_ranks, torch.ones(batch_size, num_org_q).view(-1))
    ...
    metrics = {
        ...
        "rank_accuracy": Accuracy(output_transform=lambda x: (x[0][1], x[1][1])),
        "em": Accuracy(output_transform=lambda x: (x[0][2], x[1][2])),
        "f1": MeanAbsoluteError(output_transform=lambda x: (x[0][3], x[1][2]))
    }
    ...
    if args.local_rank in [-1, 0]:
        ...
        def scoring_fn(engine):
            return engine.state.metrics["em"]

        checkpoint_handler = ModelCheckpoint(args.output_dir, 'checkpoint', n_saved=15, require_empty=False,
                                             score_function=scoring_fn)
        evaluator.add_event_handler(Events.EPOCH_COMPLETED, checkpoint_handler, {'mymodel': getattr(model, 'module', model)})  # "getattr" take care of distributed encapsulation
